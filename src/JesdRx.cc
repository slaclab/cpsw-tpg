...
#define ADD_UN(name,addr,lsbit,nbits) {                                 \
    f = IIntField::create(#name, nbits, false, lsbit);                  \
    v->CMMIODevImpl::addAtAddress(f, addr);                             \
  }
#define ADD_U1(name,addr,lsbit) ADD_UN(name,addr,lsbit,1)
...
JesdRx IJesdRx::create(const char* name) 
{
  ...
  ADD_UN (Enable       ,0x00,0,6);
  ADD_UN (SysrefDelay  ,0x04,0,5);
  ADD_UN (SubClass     ,0x10,0,1);
  ADD_UN (ReplaceEnable,0x10,1,1);
  ADD_UN (ResetGTs     ,0x10,2,1);
  ADD_UN (ClearErrors  ,0x10,3,1);
  ADD_UN (InvertSync   ,0x10,4,1);
  ADD_UN (ScrambleEnable,0x10,5,1);
  ADD_UN (LinkErrMask   ,0x14,0,6);
  ADD_RO (GTReady       ,0x40,0,6);
  ADD_RO (DataValid     ,0x40,1,6);
  ADD_RO (AlignErr      ,0x40,2,6);
  ADD_RO (nSync         ,0x40,3,6);
  ADD_RO (RxBuffUfl     ,0x40,4,6);
  ADD_RO (RxBuffOfl     ,0x40,5,6);
  ADD_RO (PositionErr   ,0x40,6,6);
  ADD_RO (RxEnabled     ,0x40,7,6);
  ADD_RO (SysRefDetected,0x41,0,6);
  ADD_RO (CommaDetected ,0x41,1,6);
  ADD_RON(DisparityErr  ,0x41,2,4,6);
  ADD_RON(DecodeErr     ,0x41,6,4,6);
  ADD_RON(DecodeErr     ,0x41,6,4,6);

  ADD_RO (SeqAddrLen ,0x000,1,1);
...
CJesdRxImpl::CJesdRxImpl(Key& key, const char* name) : CMMIODevImpl(key, name, 0x10000000, LE)
{
}
